<template>
	<div class="pic_ad_content" :style="{height:data.imgListL[0].height+'px',display:'block'}">
		<div class="img_content" @click="addedit()" :style="{height:data.imgListL[0].height+'px',display:'block'}">
				...
				<div class="swiper-wrapper">
					<transition-group tag="div" name="image">
						<div class="swiper-slide" v-show="index===mark" v-for="(item,index) in data.imgListL" v-bind:key="index">
							<img :src="item.url" draggable="false" :style="{height:data.imgListL[0].height+'px'}">
		...
		<div class="edit1 edit" v-show="data.state == 'active'">
			<h3 class="edit1_head">单页图片</h3>

			<ul class="edit1_item_group" id="uuid1">
				<li v-for="(item,index) in data.imgListL" v-bind:key="index" >
					<div class="edit1_item">
						...
						<div class="edit1_item_right" style="height: 115px;">
							<label for>标题：</label>
							...
							<br>
							<label for>高度：</label>
							<input type="number" v-model="item.height">
							<br>
							<br>
						</div>
