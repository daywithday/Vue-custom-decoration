<template>
  <div class="full_text_content">
    <div @click="addedit()" class="text_content">
      <div v-if="data.tamplate=='top'"><!--为什么有的子组件可以直接使用父组件的data属性（data.tamplate）而有的子组件却不行（item.url）子组件中的所有 props中的数据，都是通过父组件传递给
子组件的，是只读的，但为什么可以双向绑定-->
        <div class="title_group1">
          <div class="title_group_item1"
               style="color: red;border-bottom: 1px solid red;padding: 0 5px;box-sizing: border-box;">
            全部
          </div>
      ...
      <div class="goods_group4" v-if="data.listStyle == 'goods-info'">
        <div class="goods_group_item1" v-for="i in forNum">
          <img :src="goodlisg.imgUrl"/>
          <div>
            <p>{{goodlisg.name}} </p><!--加个换行符莫名其妙样式变没了-->
            <br/>
            <br/>
            <br/>
      ...
      <div class="edit2 edit">
        <h3 class="edit_head">商品分组</h3>
        <div class="edit_item_1">
        <label>列表样式：</label>
        <input type="radio" :name="'tamplate'+randomId" :id="'tamplate1'+randomId" value="top" v-model='data.tamplate' /><label :for="'tamplate1'+randomId">顶部固定菜单</label>&#x3000;
        <input type="radio" :name="'tamplate'+randomId" :id="'tamplate2'+randomId" value="left" v-model='data.tamplate' /><label :for="'tamplate2'+randomId">左侧菜单</label>&#x3000;
      </div>
        <div class="edit_item_1">
          <label for="">列表样式：</label>
          <input type="radio" :name="'listStyle'+randomId" :id="'listStyle1'+randomId" value="goods-big" v-model='data.listStyle' /><label :for="'listStyle1'+randomId">大图</label>&#x3000;
          <input type="radio" :name="'listStyle'+randomId" :id="'listStyle2'+randomId" value="goods-small" v-model='data.listStyle' /><label :for="'listStyle2'+randomId">小图</label>&#x3000;
          <input type="radio" :name="'listStyle'+randomId" :id="'listStyle3'+randomId" value="goods-info" v-model='data.listStyle' /><label :for="'listStyle3'+randomId">详细</label>
        </div>
        <div class="group_item" v-for="(itm,index) in data.goods_group" v-bind:key="index" v-dragging="{ item: itm, list: data.goods_group, group: 'itm' }">
          <div>
            <label for="">分组名称：</label> <input type="" name="" id="" value="" placeholder="单行输入" v-model="itm.title" maxlength="10" /></div>
          <div>
            <label for="">商品分组：</label>
            <a style="text-decoration: none;cursor: pointer;" data-toggle="modal" :data-target="'#myModal'+randomId">{{itm.name}}</a>
          </div>
          <div>
        ...
        <div class="group_item">
          <p style="text-align: center;cursor: pointer;" v-if="!full_group" data-toggle="modal" :data-target="'#myModal'+randomId">添加 <img src="../../static/img/u416.png" /></p>
        </div>
        <div>
          <span style="color: gray;">请优先勾选希望展示的分组，进行分组名称自定义。</span>
        </div>
        <!-- 这是 bootstrap 弹窗 里面放上列表和事件结果 对应的放入商品到 data.goodsList 即可-->
        <div class="modal fade" :id="'myModal'+randomId" tabindex="-1" role="dialog" :aria-labelledby="'myModalLabel'+randomId" aria-hidden="true">
              ...
              <div class="modal-body">
                <div class="table table-hover">
                  ...
                  <div style="text-align: right;">
                    ...
                    <button v-if="(currentPage + 1)<totalPage" @click="pageTo(currentPage - 1)">{{currentPage + 1}}</button>
