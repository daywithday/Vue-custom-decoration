					...
					<div class="edit1_item">
						<div class="edit1_item_left">
							...
							<div class="edit1_item_left_change" v-show="item.changeBtn">
								<label :for="''+randomId+index">图片地址：</label>
                <input type="text" name="" :id="''+randomId+index" value="" v-model="item.url" @blur="hideChangeBtn(index)" />
							</div>
						</div>
...
//			},
        ...
        layer.open({
          ...
          title: '图片库',
			...
			hideChangeBtn(n) {//$set()方法调用时，页面会全部更新一遍。
				var that = this
